...
if( !defined('REY_DEV_MODE') ){
	define( 'REY_THEME_VERSION', '1.7.0' );
}
else {
	// cache buster
...
add_action( 'after_setup_theme', function() {
	...
	add_action('wp_enqueue_scripts', function() {
		...
		// Main Styles
		wp_enqueue_style('rey-wp-style', get_template_directory_uri() . '/style' . $suffix . $rtl . '.css', false, REY_THEME_VERSION);
		...
		$rey_styles = apply_filters(
			...
			[
				'rey-blog'      => [
					'src'     => REY_THEME_URI . '/assets/css/rey-blog' . $suffix . $rtl . '.css',
					'deps'    => ['rey-wp-style'],
