	...
	public function mobile_gallery( $image_ids = [], $product_id = 0 )
	{
		...
		if( $product && !empty($gallery_image_ids) ){
			...
			foreach ($gallery_image_ids as $key => $gallery_img_id) {
				...
				if( $gallery_img ){
					$gallery_image = sprintf('<div><img class="woocommerce-product-gallery__mobile--%5$s" src="%1$s" data-index="%2$s" data-no-lazy="1" data-skip-lazy="1" data-full=\'%3$s\' title="%4$s"/></div>',
						$gallery_img[0],
						$key + 1,
					...
					$is_last = ($key + 1) === count($gallery_image_ids);

					$gallery_images[] = apply_filters('reycore/woocommerce/product_mobile_gallery/html', $gallery_image, $gallery_img_id, $is_last);
				}
			}
		}
