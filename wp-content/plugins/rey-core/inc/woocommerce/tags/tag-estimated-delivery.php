...
if( !class_exists('ReyCore_WooCommerce_EstimatedDelivery') ):
	...
	class ReyCore_WooCommerce_EstimatedDelivery
	{
		...
		public function display(){
			...
			$days_individual_product = reycore__acf_get_field( 'estimated_delivery__days' );

			if( $days_individual_product == -1 ){
				return;
			}
			...
			echo '<div class="rey-estimatedDelivery">';
				...
				if( $days_individual_product == '0' ){
					printf('<span class="rey-estimatedDelivery-date">%s</span>', esc_html__('Today', 'rey-core') );
				}
					...
					if( $display_type === 'date' ){
						...
						if( $margin ){
							...
							$margin_excludes = $this->settings['margin_excludes'] ? $this->settings['margin_excludes'] : get_theme_mod('estimated_delivery__exclude', ["Saturday", "Sunday"]);

							$margin_date = ' - ' . $this->calculate_date(strtotime('today'), absint($days) + absint($margin), $margin_excludes);
						}
