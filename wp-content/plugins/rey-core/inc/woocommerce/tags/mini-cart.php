...
if(!function_exists('reycore_wc__mini_cart_quantity')):
	...
	function reycore_wc__mini_cart_quantity($html, $cart_item, $cart_item_key)
	{
		...
		$product_price = apply_filters( 'woocommerce_cart_item_price', WC()->cart->get_product_price( $cart_item['data'] ), $cart_item, $cart_item_key, 'mini-cart' );

		if( apply_filters('reycore/woocommerce/minicart/item_subtotal', false) && isset($cart_item['line_total']) ){
			$product_price .= sprintf('<span class="woocommerce-Price-amount --line-total">%s</span>',wc_price( $cart_item['line_total'] ) );
		}
