	...
	function reycore__svg_arrows( $args = [] )
	{
		...
		if( $args['type'] === '' ){
			...
			if( $args['single'] ){
				...
				$arrowsSvg = reycore__arrowSvg(false, $args['class']) . reycore__arrowSvg(true, $args['class']);
			}
...
if(!function_exists('reycore__scroll_to_top')):
	...
	function reycore__scroll_to_top()
	{
		...
		printf('<a href="#scrolltotop" class="rey-scrollTop --%s">%s</a>', $style, $html);
...
class ReyCore_Walker_Nav_Menu extends Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
		...
		if( (reycore__acf_get_field('enable_menu_item_icon', $item->ID) && $icon_id = reycore__acf_get_field('menu_item_icon', $item->ID)) ){
...
if(!function_exists('reycore__cookie_notice')):
	...
	function reycore__cookie_notice()
	{
		...
		if( get_theme_mod('cookie_notice__enable', '') === '' ){
			return;
		} ?>
	...
	add_action('wp_footer', 'reycore__cookie_notice');
endif;
