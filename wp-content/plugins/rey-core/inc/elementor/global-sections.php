	...
	class ReyCore_GlobalSections
	{
		...
		public function add_gs_notices(){
			...
			if( get_post_type() === self::POST_TYPE ):
				...
				$msg = sprintf( 'You\'re editing a <strong>%s</strong> Global Section. You can change its type by accessing <span class="rey-openPageSettings">Page Settings <i class="eicon-cog" aria-hidden="true"></i></span> (bottom left corner), or go to <a href="%s" target="_blank">Global Sections</a> .', ucfirst($gs), admin_url('edit.php?post_type=rey-global-sections') );
			endif;
