	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} .rey-toggleBoxes' => '--box-distance:{{VALUE}}px;',
				]
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .rey-toggleBox' => 'text-align: {{VALUE}};',
				],
			]
		);
	...
	protected function render() {
		...
		if( !empty($settings['items']) ){
			foreach ($settings['items'] as $key => $item) {
				...
				printf('<%2$s class="rey-toggleBox %1$s" %3$s>', ($is_active ? '--active': ''), $tag, $link_attributes);

					if( $icon = $item['icon'] ){
						\Elementor\Icons_Manager::render_icon( $icon, [ 'aria-hidden' => 'true' ] );
					}

					printf('<span class="rey-toggleBox-text-main" tabindex="-1">%s</span>', $item['text']);
