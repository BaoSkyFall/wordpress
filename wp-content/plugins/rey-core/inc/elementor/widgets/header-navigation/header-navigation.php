...
class ReyCore_Widget_Header_Navigation extends \Elementor\Widget_Base {
	...
	protected function _register_controls() {
		...
		$this->add_control(
			'breakpoint',
			[
				'label'       => __( 'Breakpoint for mobile navigation', 'rey-core' ) . ' (px)',
		...
		$this->add_control(
			'submenus_distance',
			[
				'label' => esc_html__( 'Submenus Distance', 'rey-core' ) . ' (px)',
				'type' => \Elementor\Controls_Manager::NUMBER,
				'default' => '',
				'min' => 0,
				'max' => 1000,
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Custom Text - Position', 'rey-core' ),
				'type' => \Elementor\Controls_Manager::SELECT,
				'default' => 'right',
				'options' => [
					'left'  => esc_html__( 'Left', 'rey-core' ),
	 ...
	 */
	public function get_breakpoint_css( $bp )
	{
		$css = '<style>';
		...
		printf( $css, $this->get_id(), $bp );
	}

	function set_nav_options( $args ){
		...
		if( $settings['custom'] ){
			$args['mobile_menu'] = $settings['enable_mobile'] !== '' ? $settings['mobile_menu_source'] : false;
			$args['menu'] = $settings['menu_source'];
			$args['nav_id'] = '-' . $this->get_id();
			$args['override'] = true;
		}

		$args['nav_indicator'] = $settings['nav_indicator'];
	...
	protected function render() {
		...
		// Social Icons
		add_action('rey/mobile_nav/footer', function() use ($settings) {
		...
		if( $settings['custom'] && $settings['enable_mobile'] !== '' && $settings['mobile_menu_source'] && $settings['breakpoint'] ){
			$this->get_breakpoint_css( $settings['breakpoint'] );
		}
			...
			<div class="rey-mobileNav--footerItem rey-mobileNav-social">
				...
				<div class="rey-mobileNav-socialIcons">
					...
					foreach ( $social_icon_list as $index => $item ):
						...
						?>
						<a class="rey-mobileNav-socialIcons-link" <?php echo $this->get_render_attribute_string( $link_key ); ?>>
