...
if( !class_exists('ReyCore_Demos') ):
	...
	final class ReyCore_Demos
	{
		...
		private function __construct()
		{
			...
			add_action('reycore/ocdi/after_buttons', [$this, 'add_notices']);
		...
		function add_notices(){
			...
			if ( version_compare( phpversion(), $min_version, '<=' ) ) {
				echo '<div class="rey-adminNotice --error">';
				printf( __( 'The <strong>One Click Demo Import</strong> plugin requires <strong>PHP %2$s+</strong> to run properly. Please contact your hosting company and ask them to update the PHP version of your site to at least PHP %1$s.<br> Your current version of PHP is <strong>%1$s</strong>.', 'rey-core' ), phpversion(), $min_version );
				echo '</div>';
			}
		}
