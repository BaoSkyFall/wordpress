...
ReyCoreKirki::add_field( 'rey_core_kirki', [
	...
	'fields' => [
		'gs' => [
			...
			'choices'     => class_exists('ReyCore_GlobalSections') ? ReyCore_GlobalSections::get_global_sections('cover', ['' => '- Select -']) : [],
		],
		'categories' => [
			'type'        => 'select',
			'label'       => esc_html__('Categories', 'rey-core'),
			'choices'     => function_exists('reycore_wc__product_categories') ? reycore_wc__product_categories([
				'hide_empty' => false,
				'labels' => true,
			]) : [],
			'multiple' => 100
...
ReyCoreKirki::add_field( 'rey_core_kirki', [
	...
	'fields' => [
		...
		'tags' => [
			...
			'choices'     => function_exists('reycore_wc__product_tags') ? reycore_wc__product_tags([
				'hide_empty' => false,
			]) : [],
			'multiple' => 100
		],
