...
foreach ( $product_ids as $product_id ) :
	...
	if ( function_exists('reycore_wc__get_discount') && ($discount_perc = reycore_wc__get_discount($product)) ) {
		$discount = sprintf( '<span class="rey-discount">-%d%%</span>', $discount_perc );
	}
...
<div class="rey-comparePage-wrapper">
	<div class="rey-comparePage-scroll">
		<table class="rey-comparePage-table" style="--products-count: <?php echo count($products_data) ?>">
			<thead>
				<tr>
					...
					foreach ($products_data as $product_id => $product) :
						...
						<th data-id="<?php echo $product_id; ?>">
							<?php echo $link['start'] . $product['image'] . $link['end']; ?>
			...
			<tbody>
				...
				foreach ($product_fields as $field => $name) :
					...
					<tr class="__field-<?php echo esc_attr($field); ?>">
						...
						<?php
						foreach ($products_data as $product_id => $product) :
							$cell_content = isset($product[$field]) && !empty($product[$field]) ? $product[$field] : '&mdash;';
