...
if (!function_exists('reyajaxfilter_terms_output')):
	...
	function reyajaxfilter_terms_output($args) {
		...
		if ( $args['show_children_only'] && $args['taxonomy'] === 'product_cat' && is_tax($args['taxonomy'])) {
		...
		if( sizeof($args['selected_term_ids']) > 1){
			// disabled because it actually doesn't make sense
			...
			if ($sub_term_data && ($sub_term_data->parent == $args['parent_term_id'])) {
				...
				if ($count > 0 || $force_show) {
					...
					if (($args['enable_hierarchy'] && !$args['show_children_only']) || ($args['show_children_only'] && $force_show)) {
						...
						$_sub_term_ids = reyajaxfilter_get_term_childs($_parent_term_id, $args['taxonomy'], $args['hide_empty'], $args['order_by'] );
					...
					if( ! $args['dropdown'] ){
						...
						$link_attributes[] = sprintf('data-value="%s"', esc_attr($_parent_term_id));
						$link_attributes[] = sprintf('data-multiple-filter="%s"', esc_attr($args['enable_multiple']));
						$link_attributes[] = sprintf('data-slug="%s"', esc_attr($sub_term_data->slug));

						if( is_product_category() && $args['taxonomy'] === 'product_cat' ){

							$link_attributes[] = reyajaxfilter_categories_behavior_in_categories([
								'ancestors' => $ancestors,
								'sub_term_ids' => $_sub_term_ids,
								'term_data' => $sub_term_data,
								'active' => $force_show
							]);
...
if(!function_exists('reyajaxfilter_categories_behavior_in_categories')):
	/**
	 * undocumented function summary
	 *
	 * @since 1.7.0
	 **/
	function reyajaxfilter_categories_behavior_in_categories( $args = [] )
	{
		$args = wp_parse_args($args, [
			'ancestors' => [],
			'sub_term_ids' => [],
		...
		$current_is_ancestor_of_selected = in_array( $q_object->term_id, $args['ancestors'], true );
		$is_last_level = empty($args['sub_term_ids']) && in_array( $q_object->parent, $args['ancestors'], true );

		if( $is_last_level ){

			// we're in same parent category, so you should allow default ajax behavior
			if( $q_object->parent === $args['term_data']->parent ){

				$link_attribute = 'data-category-behavior="3"';

				if( $debug ){
					$link_attribute .= ' style="color: green !important;"';
				}
			}
			// other last level cats. from other parent categories
			// should redirect to target
			else {
				$link_attribute = 'data-category-behavior="1"';
				if( $debug ){
					$link_attribute .= ' style="color: navy !important;"';
				}
			}
			...
			/**
			 * It's a category outside of this active one.
			 * Used when clicking on a category that's outside the current cat. children
			 * to just redirect to it.
			 */
			if( ! $current_is_ancestor_of_selected ){
				$link_attribute = 'data-category-behavior="1"';
				if( $debug ){
					$link_attribute .= ' style="color: blue !important;"';
				}
			}
			/**
			 * Is a child of active category.
			 * It'll ajax filter, but redirect to it.
			 */
			else {
				$link_attribute = 'data-category-behavior="2"';
				if( $debug ){
					$link_attribute .= ' style="color: red !important;"';
				}
			}
		}

		if( is_tax('product_cat', $args['term_data']->term_id) && $args['active'] ){
			$link_attribute .= ' data-back="true"';
		}
...
if (!function_exists('reyajaxfilter_list_terms')):
	...
	function reyajaxfilter_list_terms($args) {
		...
		foreach ($args['terms'] as $parent_term_id) {
			...
			$should_display = apply_filters('reycore/ajaxfilters/force_empty', $count > 0, $args );

			if ( $should_display || $force_show ) {
				...
				// get sub term ids for this term
				if (($args['enable_hierarchy'] && !$args['show_children_only']) || ($args['show_children_only'] && $force_show)) {
					$sub_term_ids = reyajaxfilter_get_term_childs($parent_term_id, $args['taxonomy'], $args['hide_empty'], $args['order_by']);
				}
				...
				$link_attributes = [];
				$link_attributes[] = sprintf('data-key="%s"', esc_attr($args['data_key']));
				$link_attributes[] = sprintf('data-value="%s"', esc_attr($parent_term_id));
				$link_attributes[] = sprintf('data-multiple-filter="%s"', esc_attr($args['enable_multiple']));
				$link_attributes[] = sprintf('data-slug="%s"', esc_attr($parent_term->slug));

				if( is_tax($args['taxonomy'], $parent_term_id) && $force_show ){
					$link_attributes[] = 'data-back="true"';
				}
		...
		if (!empty($args['terms'])) {
			...
			$html .= '<div class="reyajfilter-dropdown-nav">';
				...
				foreach ($args['terms'] as $parent_term_id) {
					...
					if ($should_display || $force_show === true) {
